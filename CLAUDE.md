...
- **Tests must never touch the real user config or sync directory.** `AppConfig::save()` / `load()` resolve the live XDG path; a test that reaches them rewrites the config of whoever runs the suite. This actually happened — `apply_sync_dir` looked the path up internally, so every `cargo test` repointed the developer's `sync_dir` at a temp directory that was deleted moments later (MCC-51). Take the destination as a parameter and use `save_to` / `load_from` in tests.
